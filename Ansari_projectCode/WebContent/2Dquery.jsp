...
<% 
	...
	try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		//Make a query
   		String query1 = "select count(bill_id), date_format(time, '%H') as hourtime from bills where bar = \"" + newBar + "\" group by hourtime order by hourtime";
   		String query2 = "select count(bill_id), day from bills where bar = \"" + newBar + "\" group by day";
   		...
   		if (entity.equals("week")) {
   			...
   			while (result.next()) { 
   	   			map=new HashMap<String,Integer>();
   	   	   			map.put(result.getString("hourtime"),result.getInt("count(bill_id)"));
   	   			list.add(map);
   	   	    } 
   	   	    result.close();
   		}
